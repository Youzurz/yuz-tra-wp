...
class YUZ_Google_Translate_Adapter implements TranslateAdapterInterface {
    ...
    public function translate($text, $source_lang, $target_lang, $settings): ?string {
        ...
        yuz_tra_debug_log('YUZ-TRA: [DO] Translating using Google Translate API at ' . current_time('mysql'));

        $api_key = !empty($settings['api_key']) ? (string) $settings['api_key'] : (string) $this->apiKey;
        if (empty($api_key)) {
            yuz_tra_debug_log('YUZ-TRA: [ERROR] Google Translate API key not provided');
            return null;
        }
        ...
        if (is_wp_error($response)) {
            yuz_tra_debug_log('YUZ-TRA: [ERROR] Google Translate API request failed: ' . $response->get_error_message());
            return null;
        }
        ...
        if (isset($data['error'])) {
            yuz_tra_debug_log('YUZ-TRA: [ERROR] Google Translate API error: ' . $data['error']['message']);
            return null;
        }

        // Extract the translated text
        if (!isset($data['data']['translations'][0]['translatedText'])) {
            yuz_tra_debug_log('YUZ-TRA: [ERROR] Google Translate API response invalid: ' . print_r($body, true));
            return null;
        }

        yuz_tra_debug_log("YUZ-TRA: [CHECK] Google Translate translation succeeded: " . $data['data']['translations'][0]['translatedText'] . ' at ' . current_time('mysql'));
        yuz_tra_debug_log("YUZ-TRA: [ACT] Google Translate translation completed successfully at " . current_time('mysql'));
...
public function test_api_conn(array $settings): bool
{
    ...
    yuz_tra_debug_log('YUZ-TRA: [DO] Testing Google Translate connection at ' . current_time('mysql'));

    // Récupère la clé API (injectée ou passée en settings)
    $apiKey = $settings['api_key'] ?? $this->apiKey;
    if (empty($apiKey)) {
        yuz_tra_debug_log('YUZ-TRA: [ERROR] Google Translate API key not provided for connection test');
        return false;
    }
    ...
    if (is_wp_error($response)) {
        ...
        yuz_tra_debug_log("YUZ-TRA: [CHECK] Google Translate connection test failed: " . $error_message . ' at ' . current_time('mysql'));
        yuz_tra_debug_log("YUZ-TRA: [ACT] Google Translate connection test failed at " . current_time('mysql'));
        return false;
    }
    ...
    if ($code !== 200 || isset($data['error'])) {
        ...
        yuz_tra_debug_log("YUZ-TRA: [CHECK] Google Translate connection test failed with status: " . $code . ' - ' . $message . ' at ' . current_time('mysql'));
        yuz_tra_debug_log("YUZ-TRA: [ACT] Google Translate connection test failed at " . current_time('mysql'));
        return false;
    }

    yuz_tra_debug_log("YUZ-TRA: [CHECK] Google Translate connection test succeeded at " . current_time('mysql'));
    yuz_tra_debug_log("YUZ-TRA: [ACT] Google Translate connection test completed successfully at " . current_time('mysql'));
    return true;
}
}
