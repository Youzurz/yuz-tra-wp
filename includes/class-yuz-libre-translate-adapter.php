...
if (!class_exists('YUZ_Libre_Translate_Adapter')) {
    class YUZ_Libre_Translate_Adapter extends YUZ_Translate_Adapter implements TranslateAdapterInterface {
        private $httpClient;
        private $config;
        private function is_wporg_build(): bool {
            return defined('YUZ_TRA_WP_ORG_BUILD') && YUZ_TRA_WP_ORG_BUILD;
        }
        private function normalize_endpoint(array $settings): string {
            $endpoint = rtrim((string) ($settings['endpoint'] ?? ''), '/');
            return $endpoint;
        }
        private function append_api_key(array $payload, array $settings): array {
        ...
        public function translate(string $text, string $source_lang, string $target_lang, array $settings): ?string {
            ...
            // Validate endpoint
            $endpoint = $this->normalize_endpoint($effective_settings);
            if (empty($endpoint)) {
                yuz_tra_adapter_log('YUZ-TRA: [ERROR] No LibreTranslate endpoint configured');
                return null;
            }
            // Construct URL
            ...
            $args = [
                ...
                'body' => wp_json_encode($this->append_api_key([
                    'q' => $text,
                    'source' => $source_lang,
                    'target' => $target_lang,
                    'format' => 'text',
        ...
        public function translate_batch(array $texts, string $source_lang, string $target_lang, array $settings): array {
            ...
            $endpoint = $this->normalize_endpoint($effective_settings);
            if (empty($endpoint)) {
                yuz_tra_adapter_log('YUZ-TRA: [ERROR] No LibreTranslate endpoint configured for batch translation');
                return [];
            }
            $url = preg_match('#/translate/?$#', $endpoint) ? $endpoint : $endpoint . '/translate';
            $payload = $this->append_api_key([
