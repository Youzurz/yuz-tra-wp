/* =======================================================================================
 *  YUZ String Translation Editor - build corrigé (interop ESM⇄UMD + normalisation jQuery)
 *  Corrige: Uncaught TypeError: can't access property "use", ... .default is undefined
 *  + Politique NONCE (yuz_api_nonce remplace yuz_tst_nonce) + timeouts par catégorie
 *  + SUPPRIME toutes références CDN (jsDelivr) & plumbing d'alias
 * ======================================================================================= */

// 🔁 Remplace les imports/externals CDN par des imports de packages
import Vue from 'vue';
import VueRouter from 'vue-router';
import $ from 'jquery';
import 'select2'; // side-effect: étend $.fn.select2
import he from 'he';

import { API_CONFIG, ajaxOptionsFor, nonceFields, translateNow } from './yuz-translation-service';
...
function bootstrap() {
  ...
  const container = window.YUZ_Assets?.ensureContainer?.('yuz-editor-container') || document.getElementById('yuz-editor-container');
  if (container) {
    window.YUZ_StringTranslationApp = new Vue({
      components: { 'yuz-string-translation': StringTranslation },
    ...
    window.YUZ_Assets?.log_colored?.('critical', 'Failed to ensure yuz-editor-container');
    alert('String translation editor cannot initialize: container setup failed.');
  }
}

function waitForConfig() {
  window.YUZ_Assets?.log_colored?.('info', 'waitForConfig start');
  if (window.yuzTraSettings) {
    bootstrap();
  } else {
    // On attend uniquement la config (les deps sont importées via ESM)
